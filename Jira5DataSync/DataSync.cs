...
using Inflectra.SpiraTest.PlugIns.Jira5DataSync.SpiraImportExport;
...
namespace Inflectra.SpiraTest.PlugIns.Jira5DataSync
{
    ...
    public class DataSync : IDataSyncPlugIn
    {
        ...
        private List<JiraCustomFieldValue> ProcessJiraIssueCustomFields(int projectId, string productName, JiraIssue jiraIssue, RemoteCustomProperty[] customProperties, Dictionary<int, RemoteDataMapping> customPropertyMappingList, RemoteDataMapping[] userMappings, SpiraImportExport.ImportExportClient spiraImportExport, Dictionary<int, RemoteDataMapping[]> customPropertyValueMappingList, RemoteArtifact remoteArtifact)
        {
                        ...
                        if (customProperty.CustomPropertyTypeId == (int)Constants.CustomPropertyType.List)
                        {
                            ...
                            if (externalKey == JIRA_SPECIAL_FIELD_RESOLUTION)
                            {
                                ...
                                else
                                {
                                    ...
                                    SpiraImportExport.RemoteDataMapping customPropertyValueMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraIssue.Fields.Resolution.Id.ToString(), customPropertyValueMappings, false);
                                ...
                                foreach (JiraCustomFieldValue jiraCustomFieldValue in jiraCustomFieldValues)
                                {
                                    if (jiraCustomFieldValue.CustomFieldId.ToString() == externalKey)
                                    {
                                        ...
                                        if (jiraCustomFieldValue.Value != null)
                                        {
                                            ...
                                            SpiraImportExport.RemoteDataMapping[] customPropertyValueMappings = customPropertyValueMappingList[customProperty.CustomPropertyId.Value];
                                            SpiraImportExport.RemoteDataMapping customPropertyValueMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraCustomFieldValueId, customPropertyValueMappings, false);
                                            if (customPropertyValueMapping != null)
                                            {
                                                InternalFunctions.SetCustomPropertyValue(remoteArtifact, customProperty.PropertyNumber, customPropertyValueMapping.InternalId);
                                            }
                        ...
                        else if (customProperty.CustomPropertyTypeId == (int)Constants.CustomPropertyType.User)
                        {
                            ...
                            foreach (JiraCustomFieldValue jiraCustomFieldValue in jiraCustomFieldValues)
                            {
                                if ((jiraCustomFieldValue.CustomFieldId.ToString() == externalKey) && jiraCustomFieldValue.Value != null)
                                {
                                    ...
                                    {
                                        RemoteDataMapping customPropertyValueMapping = FindUserMappingByExternalKey(jiraCustomFieldValue.Value.StringValue, userMappings, spiraImportExport);
                                        if (customPropertyValueMapping != null)
                                        {
                                            InternalFunctions.SetCustomPropertyValue(remoteArtifact, customProperty.PropertyNumber, customPropertyValueMapping.InternalId);
                                        }
                            ...
                            if (externalKey == JIRA_SPECIAL_FIELD_COMPONENT)
                            {
                                if (jiraIssue.Fields.Components.Count > 0)
                                {
                                    ...
                                    foreach (JiraComponent jiraComponent in jiraIssue.Fields.Components)
                                    {
                                        ...
                                        SpiraImportExport.RemoteDataMapping[] customPropertyValueMappings = customPropertyValueMappingList[customProperty.CustomPropertyId.Value];
                                        SpiraImportExport.RemoteDataMapping customPropertyValueMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraComponentName, customPropertyValueMappings, false);
                                        if (customPropertyValueMapping != null)
                                        {
                                    ...
                                    if (jiraCustomFieldValue.CustomFieldId.ToString() == externalKey)
                                    {
                                        ...
                                        if (jiraCustomFieldValue.Value != null)
                                        {
                                            ...
                                            if (jiraCustomFieldValueNames != null && jiraCustomFieldValueNames.Count > 0)
                                            {
                                                ...
                                                {
                                                    SpiraImportExport.RemoteDataMapping[] customPropertyValueMappings = customPropertyValueMappingList[customProperty.CustomPropertyId.Value];
                                                    SpiraImportExport.RemoteDataMapping customPropertyValueMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraCustomFieldValueName, customPropertyValueMappings, false);
                                                    if (customPropertyValueMapping != null)
                                                    {
                                                        customPropertyValueIds.Add(customPropertyValueMapping.InternalId);
                                                    }
        ...
        private void ProcessJiraIssueAsIncident(int projectId, ImportExportClient spiraImportExport, JiraIssue jiraIssue, List<RemoteDataMapping> newIncidentMappings, List<RemoteDataMapping> newReleaseMappings, List<RemoteDataMapping> oldReleaseMappings, Dictionary<int, RemoteDataMapping> customPropertyMappingList, Dictionary<int, RemoteDataMapping[]> customPropertyValueMappingList, RemoteCustomProperty[] incidentCustomProperties, RemoteDataMapping[] incidentMappings, string jiraProject, JiraManager jiraManager, string productName, RemoteDataMapping[] severityMappings, RemoteDataMapping[] priorityMappings, RemoteDataMapping[] statusMappings, RemoteDataMapping[] typeMappings, RemoteDataMapping[] userMappings, RemoteDataMapping[] releaseMappings)
        {
            ...
            SpiraImportExport.RemoteDataMapping incidentMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraIssue.Key.ToString(), incidentMappings, false);
            ...
            if (incidentMapping == null)
            {
                ...
                if (!this.onlyCreateNewItemsInJira)
                {
                    ...
                    {
                        SpiraImportExport.RemoteDataMapping dataMapping = FindUserMappingByExternalKey(jiraIssue.Fields.Reporter.Name, userMappings, spiraImportExport);
                        if (dataMapping == null)
                        {
            ...
            try
            {
                ...
                if (remoteIncident != null)
                {
                    ...
                    {
                        //Need to handle newlines / carriage returns
                        if (jiraIssue.Fields.Description.Contains("\n") || jiraIssue.Fields.Description.Contains("\r"))
                        {
                            remoteIncident.Description = jiraIssue.Fields.Description.Replace("\n", "<br />").Replace("\r", "<br />");
                        }
                        else
                        {
                            remoteIncident.Description = jiraIssue.Fields.Description;
                        }
                    }
                    ...
                    {
                        SpiraImportExport.RemoteDataMapping dataMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraIssue.Fields.Priority.Id.ToString(), priorityMappings, true);
                        if (dataMapping == null)
                        {
                    ...
                    {
                        SpiraImportExport.RemoteDataMapping dataMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraIssue.Fields.Status.Id.ToString(), statusMappings, true);
                        if (dataMapping == null)
                        {
                    ...
                    {
                        SpiraImportExport.RemoteDataMapping dataMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraIssue.Fields.IssueType.Id.ToString(), typeMappings, true);
                        if (dataMapping == null)
                        {
                    ...
                    SpiraImportExport.RemoteComment[] incidentComments = null;
                    if (incidentId != -1)
                    {
                    ...
                    List<SpiraImportExport.RemoteComment> newIncidentComments = new List<SpiraImportExport.RemoteComment>();
                    if (jiraComments != null && jiraComments.Count > 0)
                    {
                        foreach(JiraComment jiraComment in jiraComments)
                        {
                            ...
                            {
                                foreach (SpiraImportExport.RemoteComment incidentComment in incidentComments)
                                {
                            ...
                            if (!alreadyAdded)
                            {
                                ...
                                int? creatorId = null;
                                SpiraImportExport.RemoteDataMapping dataMapping = FindUserMappingByExternalKey(jiraComment.Author.Name, userMappings, spiraImportExport);
                                if (dataMapping == null)
                                {
                                ...
                                SpiraImportExport.RemoteComment newIncidentComment = new SpiraImportExport.RemoteComment();
                    ...
                    {
                        SpiraImportExport.RemoteDataMapping dataMapping = FindUserMappingByExternalKey(jiraIssue.Fields.Assignee.Name, userMappings, spiraImportExport);
                        if (dataMapping == null)
                        {
                            //We can't find the matching user so log and ignore
                    ...
                    if (jiraIssue.Fields.Versions != null && jiraIssue.Fields.Versions.Count > 0)
                    {
                        ...
                        SpiraImportExport.RemoteDataMapping dataMapping = InternalFunctions.FindMappingByExternalKey(projectId, affectedJiraVersionId, releaseMappings, false);
                        if (dataMapping == null)
                        {
                    ...
                    if (jiraIssue.Fields.FixVersions != null && jiraIssue.Fields.FixVersions.Count > 0)
                    {
                        ...
                        //See if we have a mapped SpiraTest release in either the existing list of
                        //mapped releases or the list of newly added ones
                        SpiraImportExport.RemoteDataMapping dataMapping = InternalFunctions.FindMappingByExternalKey(projectId, fixJiraVersionId, releaseMappings, false);
                        if (dataMapping == null)
                        {
                    ...
                    List<JiraCustomFieldValue> jiraCustomFieldValues = ProcessJiraIssueCustomFields(projectId, productName, jiraIssue, incidentCustomProperties, customPropertyMappingList, userMappings, spiraImportExport, customPropertyValueMappingList, remoteIncident);
                    ...
                    if (severityCustomFieldId.HasValue)
                    {
                        foreach (JiraCustomFieldValue jiraCustomFieldValue in jiraCustomFieldValues)
                        {
                            ...
                            if (jiraCustomFieldId == severityCustomFieldId.Value.ToString())
                            {
                                ...
                                SpiraImportExport.RemoteDataMapping dataMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraCustomFieldValueId, severityMappings, true);
                                if (dataMapping != null)
                                {
                    ...
                    if (incidentId == -1)
                    {
                        ...
                        foreach (SpiraImportExport.RemoteComment newComment in newIncidentComments)
                        {
                        ...
                        try
                        {
                            ...
                            remoteUrl.ArtifactTypeId = 3;   //Incident
                        ...
                        //Finally add a link to the Spira incident from the JIRA issue
                        try
                        {
        ...
        private void ProcessJiraIssueAsRequirement(int projectId, ImportExportClient spiraImportExport, JiraIssue jiraIssue, List<RemoteDataMapping> newRequirementMappings, List<RemoteDataMapping> newReleaseMappings, List<RemoteDataMapping> oldReleaseMappings, Dictionary<int, RemoteDataMapping> customPropertyMappingList, Dictionary<int, RemoteDataMapping[]> customPropertyValueMappingList, RemoteCustomProperty[] requirementCustomProperties, RemoteDataMapping[] requirementMappings, string jiraProject, JiraManager jiraManager, string productName, RemoteDataMapping[] priorityMappings, RemoteDataMapping[] statusMappings, RemoteDataMapping[] userMappings, RemoteDataMapping[] releaseMappings)
        {
            ...
            if (requirementMapping == null)
            {
                ...
                if (!this.onlyCreateNewItemsInJira && InternalFunctions.FindMappingByExternalKey(projectId, jiraIssue.Key.ToString(), newRequirementMappings, false) == null)
                {
                    ...
                    remoteRequirement = new SpiraImportExport.RemoteRequirement();
            ...
            try
            {
                ...
                if (remoteRequirement != null)
                {
                    ...
                    else
                    {
                        ...
                        if (jiraIssue.Fields.Description.Contains("\n") || jiraIssue.Fields.Description.Contains("\r"))
                        {
                            remoteRequirement.Description = jiraIssue.Fields.Description.Replace("\n", "<br />").Replace("\r", "<br />");
                        }
                        else
                        {
                            remoteRequirement.Description = jiraIssue.Fields.Description;
                        }
                    }
                    ...
                    SpiraImportExport.RemoteComment[] requirementComments = null;
                    if (requirementId != -1)
                    {
                    ...
                    //Iterate through all the comments and see if we need to add any to SpiraTest
                    List<SpiraImportExport.RemoteComment> newRequirementComments = new List<SpiraImportExport.RemoteComment>();
                    if (jiraComments != null && jiraComments.Count > 0)
                    {
                        foreach (JiraComment jiraComment in jiraComments)
                        {
                            ...
                            {
                                foreach (SpiraImportExport.RemoteComment requirementComment in requirementComments)
                                {
                            ...
                            if (!alreadyAdded)
                            {
                                ...
                                //Add the comment to SpiraTest
                                SpiraImportExport.RemoteComment newRequirementComment = new SpiraImportExport.RemoteComment();
                        ...
                        || (jiraIssue.Fields.Versions != null && jiraIssue.Fields.Versions.Count > 0))
                    {
                        ...
                        //See if we have a mapped SpiraTest release in either the existing list of
                        //mapped releases or the list of newly added ones
                        SpiraImportExport.RemoteDataMapping dataMapping = InternalFunctions.FindMappingByExternalKey(projectId, jiraVersionId, releaseMappings, false);
                        if (dataMapping == null)
                        {
                    ...
                    List<JiraCustomFieldValue> jiraCustomFieldValues = ProcessJiraIssueCustomFields(projectId, productName, jiraIssue, requirementCustomProperties, customPropertyMappingList, userMappings, spiraImportExport, customPropertyValueMappingList, remoteRequirement);
                    ...
                    if (requirementId == -1)
                    {
                        ...
                        //Now add any resolutions (need to set the ID)
                        foreach (SpiraImportExport.RemoteComment newComment in newRequirementComments)
                        {
                        ...
                        try
                        {
                            string issueUrl = this.connectionString + "/browse/" + jiraIssue.Key.ToString();
                            SpiraImportExport.RemoteDocument remoteUrl = new SpiraImportExport.RemoteDocument();
                            remoteUrl.ArtifactId = remoteRequirement.RequirementId.Value;
                            remoteUrl.ArtifactTypeId = 1;   //Requirement
                            remoteUrl.Description = "Link to issue in JIRA";
                            remoteUrl.FilenameOrUrl = issueUrl;
                            spiraImportExport.Document_AddUrl(remoteUrl);
                        }
                        ...
                        //Finally add a link to the Spira requirement from the JIRA issue
                        try
                        {
                            string baseUrl = spiraImportExport.System_GetWebServerUrl();
                            ...
                            jiraManager.AddWebLink(jiraIssue.Key.ToString(), requirementUrl, productName + " " + Constants.INCIDENT_PREFIX + ":" + remoteRequirement.RequirementId.Value);
                        }
                        catch (Exception exception)
                        {
                            //Log a message that describes why it's not working
        ...
        private void ProcessIncident(int projectId, ImportExportClient spiraImportExport, RemoteIncident remoteIncident, List<RemoteDataMapping> newIncidentMappings, List<RemoteDataMapping> newReleaseMappings, List<RemoteDataMapping> oldReleaseMappings, Dictionary<int, RemoteDataMapping> customPropertyMappingList, Dictionary<int, RemoteDataMapping[]> customPropertyValueMappingList, RemoteCustomProperty[] incidentCustomProperties, RemoteDataMapping[] incidentMappings, JiraProject jiraProject, JiraManager jiraManager, string productName, RemoteDataMapping[] severityMappings, RemoteDataMapping[] priorityMappings, RemoteDataMapping[] statusMappings, RemoteDataMapping[] typeMappings, RemoteDataMapping[] userMappings, RemoteDataMapping[] releaseMappings)
        {
            ...
            if (InternalFunctions.FindMappingByInternalId(projectId, incidentId, incidentMappings) == null)
            {
                ...
                SpiraImportExport.RemoteSort associationSort = new RemoteSort();
                associationSort.SortAscending = true;
                associationSort.PropertyName = "CreationDate";
                SpiraImportExport.RemoteAssociation[] remoteIncidentAssociations = spiraImportExport.Association_RetrieveForArtifact((int)Constants.ArtifactType.Incident, incidentId, null, associationSort);
                ...
                SpiraImportExport.RemoteDocument[] remoteDocuments = null;
                if (SYNC_ATTACHMENTS)
                {
                    SpiraImportExport.RemoteSort attachmentSort = new RemoteSort();
                ...
                //Now get the issue type from the mapping
                SpiraImportExport.RemoteDataMapping dataMapping = InternalFunctions.FindMappingByInternalId(projectId, remoteIncident.IncidentTypeId.Value, typeMappings);
                if (dataMapping == null)
                {
                ...
                {
                    foreach (SpiraImportExport.RemoteDocument remoteDocument in remoteDocuments)
                    {
        ...
        public ServiceReturnType Execute(Nullable<DateTime> lastSyncDate, DateTime serverDateTime)
        {
            ...
            try
            {
                ...
                //Can uncomment these for certain situations, normally should not be used
                ...
                foreach (SpiraImportExport.RemoteDataMapping projectMapping in projectMappings)
                {
                    ...
                    JiraProject jiraProject = jiraProjects.FirstOrDefault(j => j.Key == jiraProjectKey);
                    if (jiraProject == null)
                    {
                        //We can't connect so go to next project
                        LogErrorEvent("Unable to connect to JIRA project " + jiraProjectKey + ", please check that the JIRA login '" + externalLogin + "' has the permissions to access this project", EventLogEntryType.Error);
                        continue;
                    }
                    ...
                    foreach (SpiraImportExport.RemoteIncident remoteIncident in incidentList)
                    {
                        try
                        {
                            ProcessIncident(projectId, spiraImportExport, remoteIncident, newIncidentMappings, newReleaseMappings, oldReleaseMappings, incidentCustomPropertyMappingList, incidentCustomPropertyValueMappingList, incidentCustomProperties, incidentMappings, jiraProject, jiraManager, productName, incidentSeverityMappings, incidentPriorityMappings, incidentStatusMappings, incidentTypeMappings, userMappings, releaseMappings);
                        }
                        catch (Exception exception)
                        {
                            //Log and continue execution
                    ...
                    string jqlFilter = "project = '" + jiraProject.Key.Replace("'", "''") + "' and updated >= '" + jqlDateTime + "' order by updated asc";
                    ...
                    foreach (JiraIssue jiraIssue in jiraIssues)
                    {
                        ...
                        if (jiraIssue.Fields.Project.Key == jiraProject.Key)
                        {
                            ...
                            {
                                ProcessJiraIssueAsRequirement(projectId, spiraImportExport, jiraIssue, newRequirementMappings, newReleaseMappings, oldReleaseMappings, requirementCustomPropertyMappingList, requirementCustomPropertyValueMappingList, requirementCustomProperties, requirementMappings, jiraProject.Key, jiraManager, productName, requirementImportanceMappings, requirementStatusMappings, userMappings, releaseMappings);
                            }
                            else
                            {
                                ProcessJiraIssueAsIncident(projectId, spiraImportExport, jiraIssue, newIncidentMappings, newReleaseMappings, oldReleaseMappings, incidentCustomPropertyMappingList, incidentCustomPropertyValueMappingList, incidentCustomProperties, incidentMappings, jiraProject.Key, jiraManager, productName, incidentSeverityMappings, incidentPriorityMappings, incidentStatusMappings, incidentTypeMappings, userMappings, releaseMappings);
                            }
                        }
                    }
